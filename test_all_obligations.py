"""
Comprehensive test for all 4 obligations
"""
import os
import sys
import logging
...
def run_comprehensive_test():
    print("="*100)
    print("COMPREHENSIVE OBLIGATIONS TEST")
    print("="*100)
    ...
    # 4. Run Analysis
    print(f"\n{'#':\u003c5} | {'Expected':\u003c10} | {'Actual':\u003c10} | {'Match':\u003c8} | {'Confidence':\u003c12} | {'Reason'}")
    print("-" * 100)
    ...
    for i, ob in enumerate(obligations):
        ...
        reason = result["reason"][:60] + "..." if len(result["reason"]) > 60 else result["reason"]
        ...
        print(f"{i+1:\u003c5} | {expected[i]:\u003c10} | {status:\u003c10} | {match:\u003c8} | {conf:\u003c12} | {reason}")
        
        # Print full reason for failed cases
        if status != expected[i]:
    ...
    # Summary
    print("=" * 100)
    print(f"SUMMARY: {passed} passed, {failed} failed out of {len(obligations)} obligations")
    print("=" * 100)
