  ...
  const handleExportReport = () => {
    ...
    const rows = results.map((r: any) => {
      const validationSteps =
        r.cot_steps
          ?.map((step: any) => `${step.step_name}: ${step.status} - ${step.finding}`)
          .join(' | ') || '';
    ...
    const csvContent = [...summaryRows.map((row) => row.join(',')), ...rows].join('\n');
        ...
        <div className='grid grid-cols-1 lg:grid-cols-12 gap-6'>
          ...
          <div
            className={`lg:col-span-5 pdf-highlight-${
              selectedObligation?.is_present === 'Yes' ? 'compliance' : 'non-compliance'
            }`}
          >
