...
export function TabsPanel({ selectedObligation, selectedClause, setSelectedClause }: any) {
  const [activeTab, setActiveTab] = useState<'details' | 'evidence' | 'suggestion'>('details');
  ...
  const stepDescriptions: Record<string, string> = {
    ...
    'Negative Obligation Check':
      "Detects exceptions like 'unless', 'except', which reverse meaning.",
  };
    ...
    <div className='bg-white/90 backdrop-blur-sm border border-file-upload-border rounded-xl p-5 h-[600px] flex flex-col shadow-lg'>
          ...
          <div className='flex-1 overflow-y-auto'>
            {activeTab === 'details' && (
              <div className='space-y-6 fade-in'>
                <div className='p-4 bg-linear-to-br from-backdrop-linear-primary to-backdrop-linear-secondary arrounded-xl border border-purple-200 shadow-sm'>
                  <span className='text-xs font-bold text-tabs-title-primary uppercase mb-3 flex items-center gap-2'>
                ...
                <div>
                  <h4 className='text-xs font-bold text-slate-900 mb-3 flex items-center gap-2'>
                    <ListChecks size={14} className='text-tabs-title-primary' /> Validation Steps
                  </h4>
                    ...
                    {selectedObligation.cot_steps?.map((step: any, i: number) => {
                      ...
                      return (
                        <div
                          ...
                          className={`p-2.5 rounded-lg border text-xs flex items-start gap-3 cursor-pointer transition-all duration-200 hover:shadow-sm ${
                            step.status === 'FAIL'
                        ...
                        >
                          ...
                          <div className='flex-1'>
                            ...
                            {isExpanded && (
                              <div
                                className={`mt-1.5 text-[11px] leading-relaxed opacity-90 font-normal border-t pt-1.5 ${
                                  step.status === 'FAIL'
                                    ? 'border-rose-200/50'
              ...
              <div className='space-y-4'>
                <div className='space-y-4 fade-in'>
                  ...
                  selectedObligation.supporting_clauses.length > 0 ? (
                    selectedObligation.supporting_clauses.map((clause: string, i: number) => (
                      <button
                        key={i}
      ...
      ${
                      ...
                      >
                        <div className='flex justify-between mb-2'>
                          ...
                          <CheckCircle
                            ...
                            width={30}
                            className={`${
                              selectedClause === clause ? 'text-green-600' : 'text-gray-400'
                            } transition-all duration-300 group-hover:scale-110 absolute right-5`}
                          />
                        </div>

                        <p className='text-sm text-slate-600 italic pt-1 text-left'>"{clause}"</p>
                      </button>
                    ))
                  ) : (
              ...
              <div className='fade-in'>
                {selectedObligation.suggestion ? (
                  <div className='p-4 bg-linear-to-br from-yellow-50 to-amber-50 rounded-xl border border-yellow-200 shadow-sm'>
                    ...
                    <p className='text-sm  leading-relaxed'>{selectedObligation.suggestion}</p>
                  </div>
                ) : (
                  <div className='text-center py-12 text-slate-600 text-md flex flex-col items-center'>
                    <CheckCircle size={32} className='mb-3 text-green-600 opacity-60' />
        ...
        <div className='flex-1 flex flex-col items-center justify-center text-slate-400 p-8 text-center'>
          ...
          <span className='text-slate-800 font-semibold text-xl mb-1'>No Selection</span>
          <p className='text-sm max-w-[250px]'>
            Select an obligation from the list to view detailed analysis and evidence.
          </p>
        </div>
      )}
    </div>
