...
export const ItemsList = ({ analysisData, selectedObligation, handleChangeObligation }: any) => {
    ...
    <div className='bg-white/90 backdrop-blur-sm border border-file-upload-border rounded-xl p-3 h-[600px] flex flex-col shadow-lg'>
      ...
      <div className='flex-1 overflow-y-auto space-y-3 scroll-bar'>
        {filterAnalysisData?.length > 0 ? (
              ...
              >
                ...
                <p
                  className={`text-sm font-medium ${
                    selectedObligation === res ? 'text-indigo-900' : 'text-slate-600'
                  }`}
                >
                  {res.obligation}
                </p>
          ...
          <span className='p-4 text-sm font-medium text-slate-600'>No obligations</span>
        )}
      </div>
    </div>
  );
