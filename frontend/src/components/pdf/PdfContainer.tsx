...
export function PdfContainer({ analysisData, file, selectedClause, defaultScale }: any) {
    ...
    <div className='bg-white/90 backdrop-blur-sm border border-purple-200 rounded-xl p-3 h-[600px] flex flex-col shadow-lg'>
      <div className='flex justify-between items-center mb-4 pb-3 border-b border-purple-200'>
        <span className='text-xl flex items-center gap-2'>
          ...
          <span className='text-gray-900 text-lg font-medium'>Contract Preview</span>
        </span>
        <a
      ...
      <div className='flex-1 bg-white rounded border border-purple-300 overflow-hidden relative shadow-inner'>
        {pdfUrl ? (
          <PdfViewer url={pdfUrl} selectedClause={selectedClause} defaultScale={defaultScale} />
        ) : (
          <div className='flex items-center justify-center h-full'>
            <p className='text-gray-900'>{analysisData?.full_text || 'No pdf loaded.'}</p>
          </div>
        )}
      </div>
    </div>
